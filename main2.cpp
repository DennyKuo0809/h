	...
	do {
		...
		for ( myturn=(start=='f'); b->winner()==Color::OTHER; flip_bit(myturn) ) {
			if ( myturn ) {
				...
				int win_most_idx = 0, num_most_win = 0;

				if(ml[0].first != 15) { // not pass
					for(int i = 0 ; i < ml.size() ; i++){
						
						b -> do_move(ml[i]);

						// tmp_b = *b;
						int nw = run_simulation();
						if(nw > num_most_win){
							win_most_idx = i;
							num_most_win = nw;
						}
				...
				auto m = ml[win_most_idx]; // if pass then win_most_idx = 0
#endif
...
int run_simulation(){
    ...
    for(int i = 0 ; i < 10 ; i++){ // simulation {SIM_NUM} times
        /* Copy the board */
        tmp_b = *b;

        /* Run simulation */
        while(tmp_b.winner() == Color::OTHER){
            /* get valid movement */
            auto ml = tmp_b.move_list();
